...
#print(data_sid0)
i = np.where(track_ids == 2988760)
track_ids_new = np.delete(track_ids, i)

df = pd.DataFrame(track_ids_new, columns =['track_id_clean'])
df['session_position'] = 16
df['session_length'] = 16
df['session_id'] = 0

df.to_csv('C:/Users/13528/Task 1/test_fold/log_input_20230123_000000000000.csv', index = False)
...
preddf_new = pd.read_csv('C:/Users/13528/Task 1/test_fold/log_input_20230123_000000000000.csv')

data = joblib.load(data_path+item[37:45]+'_y_pred_mtsk_larger_v6.pkl')

output_data_1, output_data_id_1 = TestPredProcessSessionCate(preddf_new)

dummy = np.zeros((444490,10))

output_data_1_new = np.append(output_data_1, dummy, axis=0)

output_data_id_1_new = np.append(output_data_id_1, dummy, axis=0)
...
X_test_1 = {
       'context': input_data_context_id,
        ...
        'end': input_data_end_id,
        'input_fea': input_data,
         'output_fea': output_data_1_new,
        'input_id': input_data_id,
        'output_id': output_data_id_1_new,
}
    
y_pred_1 = model.predict(X_test_1,batch_size=1500,verbose=1)
y_pred_1 = y_pred_1[:,:,1]
    
print(np.mean(y_pred_1))
        
#     hkl.dump(y_pred, data_path+item[37:45]+'_y_pred_mtsk_larger_v6.hkl', mode='w', compression='gzip')
joblib.dump(y_pred_1, data_path+item[37:45]+'_y_pred_mtsk_larger_v6_1.pkl')

data_1 = joblib.load(data_path+item[37:45]+'_y_pred_mtsk_larger_v6_1.pkl')
